...
# from TinyImageNetManualData import load_tiny_imagenet
# from Cal101ManualData import load_caltech101
# from FOOD101ManualData import load_food101
# from iNaturalistManualData import load_inaturalist
...
from conformal_learning.generate_score import load_or_compute_tensor_acc_vector,top_tensor_acc_vector
...
class Trainer:
  ...
  def _run_batch_IWCE(self, source, targets):
    self.model.train()
    self.optimizer.zero_grad()
    output = self.model(source)
    ...
    softmax_score = self.soft_proba(output)
    rank = calc_true_label_rank(softmax_score, targets)
    IWCE_rank_gamma = self.ags.IWCE_rank_exponent
    rank = rank.float() ** IWCE_rank_gamma
    ...
    if self.ags.train_CP_score == 'HPS':
      ...
      _, hard_size_loss, _ = Estimate_size_loss_HPS_hard(softmax_scores_2, targets[ind2], tau, device = self.local_rank, num_classes = self.num_classes, T = self.T, K = 1.0)         
      ...
      _, hard_size_loss, _ = Estimate_size_loss_APS_hard(softmax_scores_2, targets[ind2], tau, device = self.local_rank, num_classes = self.num_classes, T = self.T, K = 1.0)         
      ...
      tau = Smoothquantile(scores, alpha = self.train_alpha, device = self.local_rank)
      _, hard_size_loss, _ = Estimate_size_loss_RAPS_hard(softmax_scores_2, targets[ind2], tau, device = self.local_rank, num_classes = self.num_classes, T = self.T, K = 1.0)         

    ce_loss = self.loss_fn(output, targets)
    # print(f"Cross Entropy Loss shape: {ce_loss.shape}")
    # Left-hand side of the inequality: (rank - 1) * (ce_loss - 1)
    left_hand_side = (rank - 1) * (ce_loss - 1)
    left_hand_side_avg = left_hand_side.mean()  # Average across the batch
    # Right-hand side of the inequality: mean(ce_loss)
    right_hand_side = ce_loss.mean()  # Mean CE loss


    iwce_loss = ce_loss * rank.float()
    # print(f"[IWCE] IWCE loss: {iwce_loss}")
    ...
    if self.local_rank == 0:    
      ...
      self.assump_stats['rank_ce_product_mean'].append(left_hand_side_avg.item())
      self.assump_stats['ce_loss_mean'].append(right_hand_side.item())
    
    return loss

  @torch.no_grad()
  def _compute_rank_threshold_from_val(self):
      ...
      softmax = torch.cat(all_softmax, dim=0)
      targets = torch.cat(all_targets, dim=0)

      ranks = calc_true_label_rank(softmax, targets).cpu()
      sorted_ranks = torch.sort(ranks)[0]
      N = len(ranks)
      acc_thresh = 1.0 - self.train_alpha

      rank_threshold = sorted_ranks[-1].item()  # fallback
      for k in range(1, N + 1):
          acc = (ranks <= sorted_ranks[k - 1]).float().mean()
          if acc >= acc_thresh:
              rank_threshold = sorted_ranks[k - 1].item()
              break

      self.model.train()
      return rank_threshold

  def _run_batch_IWCE2(self, source, targets, rank_threshold):
    self.model.train()
    self.optimizer.zero_grad()
    output = self.model(source)
    ...
    left_hand_side_avg = left_hand_side.mean()  # Average across the batch
    right_hand_side = ce_loss.mean()  # Mean CE loss
    mask = (rank <= rank_threshold).float()    
    if mask.sum().item() < 1:
      iwce_loss = ce_loss * rank.float() ** IWCE_rank_gamma
    else:
      iwce_loss = ce_loss * mask * rank.float() ** IWCE_rank_gamma

    loss = iwce_loss.mean()  
    # print(f"The mean iwce loss is: {loss}")
  ...
  def _run_batch_only_CDF_gap(self, source, targets):
    ...
    if self.local_rank == 0:  
      self.stats['loss_all'].append(loss.item())
      self.stats['loss_size'].append(loss1.item())
  ...
  def _run_epoch(self, epoch):
    ...
    # threshold 
    rank_threshold = self._compute_rank_threshold_from_val()
    ...
    for i, (source, targets) in enumerate(self.train_data):
      ...
      # elif self.method == 'IWCE_Loss': #with CE only
      #   # print("[DEBUG] >>> Calling _run_batch_IWCE")
      #   # print(f"[DEBUG] loss_fn class: {type(self.loss_fn)}")
      #   loss_t = self._run_batch_IWCE(source, targets)
      #   loss_all += loss_t.item()  
      elif self.method == 'IWCE_Loss': #with CE only
        # print("[DEBUG] >>> Calling _run_batch_IWCE")
        # print(f"[DEBUG] loss_fn class: {type(self.loss_fn)}")
        loss_t = self._run_batch_IWCE2(source, targets, rank_threshold)
        loss_all += loss_t.item()  
  ...
  def train(self, max_epochs):
    ...
    text_file.write(f"score: {self.ags.cal_test_CP_score}, finetune_lr: {self.ags.finetune_lr}, weight_decay: {self.ags.finetune_weight_decay}, T: {self.ags.train_T}\n\n")
    ...
    self.assump_stats = {              
      'rank_ce_product_mean': [], 'ce_loss_mean': [], 
    }
    ...
    for e in tqdm(range(self.epochs_run, max_epochs)):
      ...
      if self.method == 'only_Inefficiency' or self.method == 'only_CDF_gap' or self.method == 'IWCE_Loss' : 
        # self._run_epoch_val_only(self.val_data)
        self._run_epoch_val_hard(self.val_data)
    ...
    if self.local_rank == 0:
      if self.method == 'only_Inefficiency' or self.method == 'only_CDF_gap':
  ...
  def _run_epoch_val_hard(self, data):
    self.model.eval()
    with torch.no_grad():
      for source, targets in data:
        source = source.to(self.local_rank)
        ...
        if self.ags.train_CP_score == 'HPS':
          ...
          scores = compute_scores_diff_RAPS(self.soft_proba(output), targets, device = self.local_rank, lambda_RAPS =.01, k_RAPS =5)
          tau = Smoothquantile(scores, alpha = self.train_alpha, device = self.local_rank)
          _, hard_size_loss, _ = Estimate_size_loss_RAPS_hard(scores, targets, self.tau, device = self.local_rank, num_classes = self.num_classes, T = self.T, K = 1.0)         

    
        if self.local_rank == 0:  
          self.eval_stats['indicator_val_hard_set_size'].append(hard_size_loss.item())
  ...
  def _save_snapshot(self, epoch, name):
    ...
    snapshot['assump_stats'] = self.assump_stats
...
def main(ags, train_sample_dataset, hout_sample_dataset, file_final, log_file):
  ...
  if ags.method == 'IWCE_Loss':
    loss_fn = torch.nn.CrossEntropyLoss(reduction = 'none')
    print(f"[DEBUG] loss_fn set to: {loss_fn}")
  ...
  if check_path(ags, file_final):
    print(f'main train check_path ags = {ags}')
...
if __name__ == "__main__":
  ...
  # print(file_final)
  print(f"normal training start file_final: {file_final}")      
  main(ags, train_sample_dataset, hout_sample_dataset, file_final, log_file)
  print(f"normal training finish log_file: {log_file}")  
  ...
  for i in range(len(black_boxes_path)):
    ...
    elif ags.cal_test_CP_score == 'SAPS':
      scores_simple_clean_test, Y_test, y_pred = get_scores_SAPS(model, test_loader_all, num_classes = ags.num_classes, lambda_SAPS =.15, device=local_rank)
      scores = scores_simple_clean_test[torch.arange(len(scores_simple_clean_test)), Y_test]
      data_frame = pd.DataFrame(
        {'score': scores}
      )
      if local_rank == 0:
        data_frame.to_csv(file_final + '/SAPS_scores.csv', index = False) 
      #exit(1)
    else:
  ...
  save_path_final = save_path1 + '/' + black_boxes_names[0] + '/'
  result_file = save_path_final + '/results_bbox.csv'

  data_bbx = pd.read_csv(result_file)

  x_1 = data_bbx['Size (mean)'].tolist()
  y_1 = data_bbx['Coverage'].tolist()
  print('ags.method')
  print(f'Size avg value = {np.mean(x_1)}')
  print(f'Size STD = {np.std(x_1)}')
  print(f'Coverage avg value = {np.mean(y_1)}')
  print(f'Coverage STD = {np.std(y_1)}')
