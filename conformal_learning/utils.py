...
def calc_true_label_rank(scores: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    """
    Calculate the rank of the true label for each sample based on the scores.
    
    For 1D tensors, it returns the rank of a single score for each sample.
    For 2D tensors, it returns the rank of the true label for each sample based on the scores across classes.

    :param scores: A tensor of shape [batch_size, num_classes] for 2D or [batch_size] for 1D containing the predicted scores.
    :param labels: A tensor of shape [batch_size] containing the true labels.
    :return: A tensor of shape [batch_size] containing the ranks of the true labels.
    """
    batch_size = scores.size(0)
    
    # Check if scores is 1D or 2D
    if scores.dim() == 1:
        # For 1D scores, each sample has a single score
        rank = torch.argsort(scores, descending=True) + 1  # Return the rank of each sample (1-based)
        return rank
    
    elif scores.dim() == 2:
        # For 2D scores, each sample has scores across multiple classes
        sorted_indices = torch.argsort(scores, dim=1, descending=True)  # [B, C]

        # Compare each sorted row to the true label, get bool matrix
        matches = sorted_indices == labels.unsqueeze(1)  # [B, C]

        # First True index along dim=1 gives the 0-based rank
        rank = matches.float().argmax(dim=1) + 1  # 1-based rank

        return rank
    
    else:
        raise ValueError(f"Expected 1D or 2D tensor for scores, but got {scores.dim()}D tensor.")
...
def generalized_inverse_quantile_score_SAPS(
    probabilities,
    labels,
    u=None,
    lambda_SAPS=0.05,
    ...
    """
    SAPS (Eq.10) non-conformity score.
      - rank==1: score = u_i * MSP_i
      - rank>=2: score = MSP_i + (rank - 2 + u_i) * lambda_SAPS

      all_combinations=True  -> (n, K)
      all_combinations=False -> (n,)
    """
    probs = np.asarray(probabilities)
    n, K = probs.shape

    if u is None:
        u_vec = np.ones(n, dtype=probs.dtype)
    else:
        u_vec = np.asarray(u, dtype=probs.dtype).reshape(n)

    order = np.argsort(-probs, axis=1)                              # (n, K)
    ranks_full = np.empty_like(order)
    ranks_full[np.arange(n)[:, None], order] = np.arange(1, K + 1)[None, :]

    top1_cols = order[:, 0]                                         # (n,)
    msp = probs[np.arange(n), top1_cols]                            # (n,)
    u_msp = u_vec * msp                                             # (n,)

    scores_full = msp[:, None] + (ranks_full.astype(np.float64) - 2.0 + u_vec[:, None]) * float(lambda_SAPS)

    scores_full[np.arange(n), top1_cols] = u_msp

    if all_combinations:
        return scores_full[:, labels]
    else:
        return scores_full[np.arange(n), labels]
...
def get_scores_SAPS(
    ...
    # return y_pred: (n,), logits: (n, K), y_true: (n,)
    y_pred, logits, y_true = get_logits_targets(model, DataLoader, num_classes, device)

    # T=1.0
    T = torch.tensor([1.0], device=logits.device, dtype=logits.dtype)

    # softmax 
    simple_outputs = layer_prob(logits / T).detach().cpu().numpy()   # (n, K)
    ...
    scores_simple = generalized_inverse_quantile_score_SAPS(
        simple_outputs,
        labels=np.arange(num_classes),
        u=uniform_variables,                 #  u=None
        lambda_SAPS=lambda_SAPS,
...
# def compute_sscv(S, y, y_pred, alpha=0.1):
#     sizes = [len(S[i]) for i in range(len(S))]
#     cover_indicator = np.array([y[i] in S[i] for i in range(len(S))])
    
#     # Group indices by size
#     strata = defaultdict(list)
#     for i, size in enumerate(sizes):
#         if size != 0:
#             strata[size].append(i)
...
#         if len(indices) == 0:
#             continue
#         cov = np.mean(cover_indicator[indices])
#         deviation = abs(cov - (1 - alpha))
#         violations.append(deviation)
#         rows.append({
#             'Set Size': size,
#             'Count': len(indices),
#             'Coverage': cov,
#             '|Coverage - (1 - alpha)|': deviation
#         })
    
#     sscv = max(violations) if violations else 0.0
#     # print('SSCV:', sscv)
    
#     return pd.DataFrame([{'SSCV': sscv}])

def compute_sscv(S, y, y_pred, alpha=0.1, strata=None):
    """
    Compute SSCV (Size-Stratified Coverage Violation) over stratum intervals.
    
    Args:
        S: list of prediction sets
        y: list or array of true labels
        y_pred: predicted labels (not used here, kept for compatibility)
        alpha: miscoverage level
        strata: list of (low, high) tuples specifying size intervals

    Returns:
        pd.DataFrame with SSCV and optional details
    """

    if strata is None:
        # Default strata: grouped by small ranges
        max_size = max(len(s) for s in S)
        strata = [(i, i+1) for i in range(1, max_size+1, 2)]  # e.g., (1–2), (3–4), ...

    # Compute size and coverage indicators
    sizes = np.array([len(S[i]) for i in range(len(S))])
    cover_indicator = np.array([y[i] in S[i] for i in range(len(S))])

    # DataFrame of all results
    df = pd.DataFrame({'size': sizes, 'correct': cover_indicator})

    violations = []
    rows = []

    for (low, high) in strata:
        temp_df = df[(df['size'] >= low) & (df['size'] <= high)]
        if len(temp_df) == 0:
            continue
        coverage = temp_df['correct'].mean()
        deviation = abs(coverage - (1 - alpha))
        violations.append(deviation)
        rows.append({
            'Stratum': f"{low}-{high}",
            'Count': len(temp_df),
    ...
    result_df['SSCV'] = sscv  # Append SSCV to all rows for reference

    return result_df


def evaluate_predictions_new(S, y, y_pred, alpha, coverage_on_label=False, num_of_classes=10):
    #y = torch.from_numpy(y)
    #y_pred = torch.from_numpy(y_pred)
    ...
    sscv = compute_sscv(S, y, y_pred, alpha)
...
def tune_RAPS(model, test_loader_all, num_classes, alpha, device, default_qhat=np.inf, regularize=False, exact_coverage=False):

    lambda_RAPS_grid = [1e-4, 1e-3, 0.01, 0.02, 0.05, 0.2, 0.5, 0.7, 1.0]
    k_RAPS_grid = [1, 5, 10, 50]

    min_apss = num_classes
    best_lambda = 1.0
    best_k = 50

    for lambda_RAPS, k_RAPS in itertools.product(lambda_RAPS_grid, k_RAPS_grid):
        scores_simple_clean_test, Y_test, y_pred = get_scores_RAPS(model, test_loader_all, num_classes, lambda_RAPS, k_RAPS, device)
        scores = scores_simple_clean_test[torch.arange(len(scores_simple_clean_test)), Y_test]

        n_test = len(Y_test)
        idx1, idx2 = train_test_split(np.arange(n_test), train_size=0.499, random_state = 1111)
    
        thresholds, THR, set_matrices = Marginal_conformal(scores_simple_clean_test[idx1, Y_test[idx1]], Y_test[idx1], scores_simple_clean_test[idx2, :], Y_test[idx2], alpha,
                          num_classes, default_qhat=default_qhat, regularize=regularize, exact_coverage=exact_coverage)

        size_mean = np.mean([len(set_matrices[i]) for i in range(len(Y_test[idx2]))])

        if size_mean < min_apss:
            min_apss = size_mean
            best_lambda = lambda_RAPS
            best_k = k_RAPS


    return best_lambda, best_k 
...
def wsc_unbiased(X, y, S, delta=0.1, M=1000, test_size=0.75, random_state=2020, verbose=False):
    ...
    result_df = pd.DataFrame([{'WSC': coverage}])

    return result_df
   
# def get_logits_targets_new(model, loader, num_classes, device):
#     device = next(model.parameters()).get_device()

#     logits = torch.zeros((len(loader.dataset), num_classes))
#     labels = torch.zeros((len(loader.dataset),))
#     features = torch.zeros((len(loader.dataset), *next(iter(loader))[0].shape[1:]))  # Initialize with shape of one sample
#     i = 0
...
#             labels[i:i + batch_size] = targets.cpu()
#             features[i:i + batch_size] = x.cpu()  # Save the inputs (features)

#             i += batch_size

#     return torch.argmax(logits, dim=1).long(), logits, labels.long(), features

# def get_scores_HPS_new(model, loader, num_classes=10, device='cpu'):
#     layer_prob = torch.nn.Softmax(dim=1)

#     y_pred, logits, y_true, features = get_logits_targets_new(model, loader, num_classes, device)

#     K = torch.tensor([1.0])  # Temperature scaling parameter
#     simple_outputs = layer_prob(logits / K).detach().numpy()

#     return 1 - simple_outputs, y_true, y_pred, features
...
def compute_avg_cov_gap(S, y, alpha=0.1):
    """
    Computes the average absolute gap between class-conditional coverage
    and target coverage (1 - alpha).

    Args:
        S: list of prediction sets (one per example)
        y: true labels (list or array)
        alpha: target miscoverage level

    Returns:
        avg_class_cov_gap: scalar, average class-conditional coverage gap
    """
    y = np.array(y)
    ...
    for cls in unique_labels:
        ...
        if len(indices) == 0:
            continue
        correct = [y[i] in S[i] for i in indices]
        class_cov = np.mean(correct)
        class_cov_list.append(class_cov)

    class_cond_cov = np.array(class_cov_list)
    avg_class_cov_gap = np.mean(np.abs(class_cond_cov - (1 - alpha))) * 100

    result_df = pd.DataFrame([{'GovGap': avg_class_cov_gap}])

    return result_df


def compute_class_stats(S, y, alpha=0.1):
    """
    Computes class-conditional coverage, average prediction set size, and class label.
    ...
    for cls in unique_labels:
        ...
        avg_size = np.mean([len(S[i]) for i in indices])
...
def get_scores_HPS_eval(model, loader, num_classes = 10, device='cpu'):
    layer_prob = torch.nn.Softmax(dim=1)
    ...
    simple_outputs = layer_prob(logits/K)

    return simple_outputs, y_true, y_pred


def eval_coverage(softmax_scores, q, Y_true, exact_coverage=False):
    
    val_scores = 1 - softmax_scores
    classwise_preds = create_classwise_prediction_sets(val_scores, q, exact_coverage=exact_coverage)
    coverage = np.mean([Y_true[i] in classwise_preds[i] for i in range(len(Y_true))])

    return coverage


def compute_size(S, y, y_pred):
    # Compute size and coverage indicators
    sizes = np.array([len(S[i]) for i in range(len(y))])
    size_std = np.std(sizes)
    print(f"Prediction set sizes std: {size_std}")
